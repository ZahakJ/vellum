...
describe("symlinks", () => {
  it("string-checks the path but does NOT resolve links (known gap)", async () => {
    // safeAbs is a lexical check: "escape.md" is inside the vault as a STRING,
    // and no realpath() follows. Reading it therefore returns a file that
    // lives outside the vault. Documented rather than asserted-away because
    // the fix (realpath + prefix check on every read/write) is a behavior
    // change: vaults that deliberately symlink a shared folder in would break.
    assert.equal(safeAbs("escape.md"), path.join(root, "escape.md"));
    const data = await readNote("escape.md");
    assert.match(data.content, /outside the vault/);
    // Same for a symlinked directory.
    assert.equal(safeAbs("escapedir/secret.md"), path.join(root, "escapedir/secret.md"));
    const viaDir = await readNote("escapedir/secret.md");
    assert.match(viaDir.content, /outside the vault/);
  });
});
