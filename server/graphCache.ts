...
function current(publishedOnly: boolean, lang: FilterLang): Memo {
  ...
  // audience alone would hand the first reader's answer to the second. That is
  // the same class of leak the admin/visitor split exists to prevent, one axis
  // over. An admin is never language-filtered, so every admin request still
  // shares the one entry.
  const key = publishedOnly ? `visitor:${lang}` : "admin";
  const hit = memo.get(key);
  if (hit) return hit;
  const data = graph(publishedOnly, lang);
...
export function graphBody(publishedOnly: boolean, lang: FilterLang): EncodedBody {
  return current(publishedOnly, lang).body;
}
 ...
 * same answer.
 */
export function localGraphJson(
  notePath: string,
  publishedOnly: boolean,
  lang: FilterLang,
): string {
  const m = current(publishedOnly, lang);
